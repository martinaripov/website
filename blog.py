...
app = Flask(__name__)
app.config['SECRET_KEY'] = 'mk@387209l32w!'
...
def post(post_id):
    ...
    for comment in result:
        info.append(
            {'id': comment[0], 'post_id': comment[1], 'user_id': comment[2], 'content': comment[3], 'username': comment[5]}
        )
    comment_dict = {'comments': info}
    return render_template('post.html', post=post_dict, **comment_dict)
